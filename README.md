...
- **Rates are taken at invoice time, not work time.** The snapshot on each invoice line protects invoices that already exist, but an approved worklog that has not been invoiced yet is billed at the assignment's *current* rate. Work done at 100/h and invoiced after a raise to 120/h is billed at 120. The production fix is an effective-dated rate on the assignment (`ValidFrom`), looked up by `WorkDate`, or snapshotting the rate onto the worklog at approval. Which one is right is a billing-policy question for the business.
